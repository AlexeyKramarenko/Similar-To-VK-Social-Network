...
function MainView() {
    ...
    this.addCommentMarkup = addCommentMarkup;
    this.addPostMarkup = addPostMarkup;
    this.addCommentToWall = addCommentToWall;
    this.addAlbumGalleryMarkup = addAlbumGalleryMarkup;
    this.addGalleryMarkup = addGalleryMarkup;
    this.changeColor = changeColor;
    this.clearMessageTextarea = clearMessageTextarea;
    this.clearAlbumContainer = clearAlbumContainer;
    this.clearContainer = clearContainer;
    this.createPostForm = createPostForm;
    this.createCommentsForm = createCommentsForm;
    this.changePostsCount = changePostsCount;
    this.deleteCommentMarkup = deleteCommentMarkup;
    this.deletePostsMarkup = deletePostsMarkup;
    this.displayCommentAnchors = displayCommentAnchorsIfCommentsCountIsNotZero;
    this.getCurrentUserId = getCurrentUserId;
    this.getLoadedImage = getLoadedImage;
    this.getCommentId = getCommentId;
    this.getComment = getComment;
    this.getCommentTemplate = getCommentTemplate;
    this.getCurrentMessage = getCurrentMessage;
    this.getUserIdOfPageOwner = getUserIdOfPageOwner;
    this.getStatus = getStatus;
    this.getStatusId = getStatusId;
    this.getInvitationMessage = getInvitationMessage;
    this.onError = onError;
    this.onSuccessCreateDialog = onSuccessCreateDialog;
    this.postMessage = getPostMessage;
    this.runModalWindowOnAvatarClick = runModalWindowOnAvatarClick;
    this.setAvatar = setAvatar;
    this.setCaretPosition = setCaretPosition;
    this.setLargeAvatar = setLargeAvatar;
    ...
    function getPostMessage() {
        return $("textarea#txtMessage").val();
    }
    ...
    function getInvitationMessage() {
        ...
        var message = {
            ...
            Body: "<p>Hello! I wanna be your friend!<br/>" +
                                      "FROM: <a href='/main/mainpage/" + currentUserId + "'>" + "--userNamePlaceHolder--" +
                                        "</a><br/>" +
    ...
    function setAvatar(url) {
        $('#imgAvatar').attr('src', ROOT + url);
    }
    function setLargeAvatar(url) {
        $('.imgLargeAvatar').attr('src', ROOT + url);
        return false;
    }
                                ...
                                '<div class="table_">' +
                                    '<div class="table_row">' +
                                        '<div class="table_leftCell">' +
                                            ...
                                            '<a href="">' +
                                                '<img id="Image3" src="' + ROOT + avatar + '" class="post_image" style="width:30px;">' +
                                            '</a>' +
                                            '<br>' +
                                        '</div>' +
                                        '<div class="table_rightCell">' +
                                            '<b>' + userName + '</b><br>' +
                                            '<a class="delete" onclick="vm.deletePost(this)">delete</a>'+
                                            postMessage +
                                        '</div>' +
                                    '</div>' +
                                '</div>' +
            ...
            innerContent += "<li style='display:none' class='list' data-photoname='" + photo.Name + "'" +
                           ...
                           "<h1><p>" + photo.Description + "</p></h1>'" +
                            "data-src='" + ROOT + photo.PhotoUrl + "'  >" +
                                "<a href=''>" +
                                   "<img width='100px' height='62px' class='img-responsive' src='" + ROOT + photo.ThumbnailPhotoUrl + "'  />" +
                                "</a>" +
                        "</li>";
        });
